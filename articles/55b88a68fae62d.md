...
スタジアムのプロダクトチームが開発している [FANTS](https://fants.jp/) は、ブランドサイト [https://fants.jp/](https://fants.jp/) の運用を WordPress で行っています。

こういったブランドサイトは、ABテストを行うことで、デザインや文言の最適化を行うことがあります。この記事では、WordPress で運用されているサイトで、ABテストを実施するためのツールの選定と、WordPress プラグインである Nelio AB Testing を用いたテストの方法について解説します。
...
A/B テスト 機能を提供する WordPress プラグインを用いる方法です。WordPress では、プラグインでできることは非常に強力で、何でもできてしまうため、良いプラグインに出会えれば簡単にA/Bテストが実現できます。

調べたところ下記のプラグインが比較的利用実績があり、良さそうでした。
...
#### Nelio AB Testing を選定

以下を検討した結果、WordPress プラグイン の Nelio AB Testing を利用することにしました。
...
- 昔使ったことがある Google Optimize が 第一候補だったが、[2023年9月に廃止されている](https://support.google.com/analytics/answer/12979939?hl=ja)。
  ...
  - A/Bテストの内容や成果がツール側に蓄積されていき、テーマ(テンプレート) の GitHub 修正との相性が悪そうだった。
- AFFINGER などテーマの変更はコストが大きい。
  - FANTS のブランドサイトは、独自の WordPress テーマとして実装されており、テーマを入れ替える手間は許容できなかった。
- WordPress プラグイン で使えそうなものがあれば良さそう。
  - A/Bテスト プラグインは複数の選択肢があるが、以下の観点で検討
  - A/Bテストできる対象: 固定ページ、記事など、A/Bテストの対象は多彩か。
  - 開発は継続的に行われているか。WordPressはアップデートが早いので、開発が止まっているプラグインは負債になりやすい。
  - ドキュメント等の情報源は豊富か。
  - 管理画面の使い勝手など、導入や管理が容易か。

いくつかのプラグインのドキュメントを読んだり、試用してみてみましたが、下記の理由で Nelio AB Testing を選択することにしました。

- Nelio AB Testing での A/Bテストの対象が非常に多彩で、固定ページや記事など、改善したいページは一通り対応していた。
- [Nelio AB Testing Support Forum](https://wordpress.org/support/plugin/nelio-ab-testing/) など、コミュニティも存在し、情報量があった。
- [Nelio Software](https://neliosoftware.com/testing/) が事業として開発していて、アップデートも頻繁に行われている。個人プラグインは継続性が心配。
...
インストール方法は、よくあるプラグインと同じです。WordPress の 管理画面にて「新規プラグインの追加」画面にて、キーワードで「Nelio A/B Testing」を入力して、インストールしてください。

インストールしたら、プラグイン一覧で、有効化 (Activate) しましょう。

有効化すると、管理画面のメニューに「Nelio A/B Testing」が表示されます。以後、このメニューから操作することになります。
...
既存のテンプレートをコピーすると楽ですが、管理画面で選択するときに重複しないように、コピー後に テンプレートファイルの冒頭の Template Name の値を変更するのを忘れずに修正しておきましょう。
...
前項で作成したテンプレートファイルに ABテスト で試したい内容を記述しましょう。
...
途中経過は、下図のようにグラフでわかりやすく表示され、十分なサンプル数が揃うと 結果 が表示されます。
この例では、なんと +94.1% の改善となり、CVRが約倍になりました。

![ABテストの結果](/images/nelio-ab-testing/5.png)

#### A/Bテストの結果を適用する

結果が確定したら、ABテストを終了して、A案が勝った場合はB案を破棄し、B案が勝った場合はA案にB案の内容を取り込みましょう。
